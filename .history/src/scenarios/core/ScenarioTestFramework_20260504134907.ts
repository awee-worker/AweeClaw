  ...
  private checkManifest(module: ScenarioModule, checks: ScenarioTestCheck[]): void {
    ...
    for (const field of requiredManifestFields) {
      checks.push({
        ...
        passed: !!(manifest as Record<string, unknown>)[field],
        message: !(manifest as Record<string, unknown>)[field] ? `Manifest field "${field}" is required` : undefined,
        severity: 'critical',
      })
    }
  ...
  private checkPlugin(module: ScenarioModule, checks: ScenarioTestCheck[]): void {
    ...
    for (const field of requiredPluginFields) {
      checks.push({
        ...
        passed: !!(plugin as Record<string, unknown>)[field],
        message: !(plugin as Record<string, unknown>)[field] ? `Plugin field "${field}" is required` : undefined,
