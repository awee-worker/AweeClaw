...
import type { ToolDefinition, ToolExecutionResult, ToolExecutionContext, ToolExecutor } from '@/shared/types'
  ...
  getPlugin(): ScenarioPlugin {
    ...
    const scenarioUI: ScenarioUI = {
      ...
      panels: ui?.panels?.map(p => {
        const knownPanels: Record<string, { id: string; component: string; region: string; defaultVisible: boolean; resizable: boolean }> = {
        ...
        return knownPanels[p] || { id: p, component: p, region: 'primary', defaultVisible: true, resizable: true }
      }) || [
