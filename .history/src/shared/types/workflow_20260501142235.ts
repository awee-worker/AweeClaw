  ...
  private async executeStep(
    ...
    try {
      ...
      if (step.config.type === 'agent_message') {
        ...
        if (agentConfig.outputVar) {
          run.variables[agentConfig.outputVar] = output
        }
      } else if (step.config.type === 'tool_call') {
        const toolConfig = step.config as ToolCallConfig
        if (toolConfig.outputVar) {
          run.variables[toolConfig.outputVar] = output
        }
      } else if (step.config.type === 'transform') {
        const transformConfig = step.config as TransformConfig
        if (transformConfig.outputVar) {
          run.variables[transformConfig.outputVar] = output
        }
      } else if (step.config.type === 'user_input') {
        const userInputConfig = step.config as UserInputConfig
        if (userInputConfig.outputVar) {
      ...
      if (step.config.type === 'parallel') {
        ...
        const parallelResults = await this.executeParallelSteps(run, parallelConfig, definition)
        if (parallelConfig.outputVar) {
          run.variables[parallelConfig.outputVar] = parallelResults
        }
      }
...
export interface WorkflowEvent {
  type: 'workflow_started' | 'workflow_completed' | 'workflow_failed' | 'workflow_paused' | 'workflow_resumed' | 'step_started' | 'step_completed' | 'step_failed'
  runId: string
  workflowId: string
  stepId?: string
  output?: unknown
