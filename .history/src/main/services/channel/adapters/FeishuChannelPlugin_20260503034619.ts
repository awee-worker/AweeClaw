...
export class FeishuChannelPlugin implements ChannelPlugin {
  ...
  async connect(account: ChannelAccountConfig): Promise<void> {
    ...
    try {
      ...
      logger.channel.info(`[Feishu] Starting WebSocket connection for account ${account.id}...`)

      await wsClient.start({
        eventDispatcher: new Lark.EventDispatcher({}).register({
          'im.message.receive_v1': async (data: any) => {
            try {
              logger.channel.info(`[Feishu] Received im.message.receive_v1 event`)
              const message = self.normalizeSdkMessage(accountId, data)
              if (message) {
                logger.channel.info(`[Feishu] Normalized message: from=${message.from}, text=${message.text?.slice(0, 50)}`)
                for (const cb of self.messageCallbacks) cb(message)
              } else {
                logger.channel.warn(`[Feishu] Failed to normalize message, data: ${JSON.stringify(data)?.slice(0, 200)}`)
              }
            } catch (err) {
      ...
      logger.channel.info(`[Feishu] Account ${account.id} connected via WebSocket successfully`)
    } catch (err) {
