...
class PythonManager {
  ...
  async executeInlineScript(params: {
    ...
    const uvPath = this._status.uvPath || resolveCommandPath('uv')
    if (!uvPath) {
      return { success: false, stdout: '', stderr: '', exitCode: null, error: 'uv not available. Please ensure Python environment is set up.' }
    }
    ...
    try {
      ...
      logger.system.info(`[PythonManager] Executing inline script via uv run: ${tempFile}`)

      const result = await execCommandAsync(
        uvPath,
        ['run', '--script', tempFile, ...args],
      ...
      return { success: false, stdout: '', stderr: errMsg, exitCode: null, error: errMsg, tempFile }
    } finally {
      try {
