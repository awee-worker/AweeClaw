...
export class McpManager extends EventEmitter {
  ...
  async connectServer(configOrId: McpServerConfig | string): Promise<void> {
    ...
    try {
      ...
      try {
        if (client.state.transport) {
          if (client.state.transport instanceof StdioClientTransport) {
            try {
              // @ts-ignore
              const subProcess = client.state.transport._process
          ...
          await client.state.transport.close().catch(() => { })
          client.state.transport = null
        }
        if (client.state.client) {
          await client.state.client.close().catch(() => { })
          client.state.client = null
        }
      } catch { /* ignore cleanup errors */ }
      // 确保 McpClient 内部状态是 error（而非被 disconnect 覆盖为 disconnected）
      if (client.status !== 'error' && client.status !== 'needs_auth' && client.status !== 'needs_registration') {
        client.updateStatusManually('error', error.message)
      }
    }
