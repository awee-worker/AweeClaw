...
export function registerChannelHandlers(getMainWindow?: () => BrowserWindow | null): void {
  if (getMainWindow) {
    channelBridge.init(getMainWindow)
  }
  ...
  safeIpcHandle('channel:sendReply', async (_, conversationKey: string, text: string) => {
    const result = await channelBridge.sendReply(conversationKey, text)
    return result
  })
