...
export function registerScenarioMarketplaceHandlers(): void {
  ipcMain.handle('scenario:marketplaceSearch', async (_event, query: string, page?: number, pageSize?: number) => {
    try {
  ...
  ipcMain.handle('scenario:marketplaceFeatured', async () => {
    try {
      const { marketplaceAPI } = await import('../../scenarios/marketplace')
  ...
  ipcMain.handle('scenario:marketplaceDetails', async (_event, scenarioId: string) => {
    try {
      const { marketplaceAPI } = await import('../../scenarios/marketplace')
  ...
  ipcMain.handle('scenario:marketplaceCategories', async () => {
    try {
      const { marketplaceAPI } = await import('../../scenarios/marketplace')
  ...
  ipcMain.handle('scenario:marketplaceDownload', async (_event, scenarioId: string) => {
    try {
      const { marketplaceAPI } = await import('../../scenarios/marketplace')
