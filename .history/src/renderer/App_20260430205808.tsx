...
function AppContent() {
  ...
  const scenarioLayout = useMemo(() => {
    const scenario = scenarioRegistry.get(activeScenarioId)
    return scenario?.ui?.layout || 'editor-centric'
  }, [activeScenarioId])

  const isChatCentric = scenarioLayout === 'chat-centric' || scenarioLayout === 'analytics-centric'
  const isFullscreenChat = scenarioLayout === 'fullscreen-chat'
  const isEditorCentric = !isChatCentric && !isFullscreenChat
