...
const SPREADSHEET_EXTENSIONS = new Set(['xlsx', 'xls', 'xlsm', 'xlsb', 'csv', 'tsv', 'ods'])
...
const FileProvider: ContextProvider = {
  ...
  async process(item, _ctx) {
    ...
    try {
      ...
      if (BINARY_EXTENSIONS.has(ext) && !SPREADSHEET_EXTENSIONS.has(ext)) {
        return `\n### File: ${filePath}\n[Binary file: ${ext.toUpperCase()} format. Use appropriate tools to process this file.]\n`
      }

      if (SPREADSHEET_EXTENSIONS.has(ext) && ext !== 'csv' && ext !== 'tsv') {
        return `\n### File: ${filePath}\n[Spreadsheet file: ${ext.toUpperCase()} format. Use the spreadsheet tool with operation "read" and path "${filePath}" to read this file.]\n`
      }
