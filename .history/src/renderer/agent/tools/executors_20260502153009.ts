...
const rawToolExecutors: Record<string, (args: Record<string, unknown>, ctx: ToolExecutionContext) => Promise<ToolExecutionResult>> = {
    async read_file(args, ctx) {
                ...
                paths.map(p => limit(async () => {
                    try {
                        ...
                        if (isBinaryPath(validPath)) {
                            ...
                            return `\n--- File: ${p} ---\n[Binary file: ${ext} format. This file cannot be read as text. Use the spreadsheet tool for Excel files, or convert to CSV/JSON for analysis.]\n`
                        }
        ...
        if (isBinaryPath(path)) {
            ...
            return {
                success: true,
                result: `[Binary file: ${ext} format. This file cannot be read as text. Use the spreadsheet tool (operation: read) for Excel files, or convert to CSV/JSON for analysis.]`,
                meta: { filePath: path },
            }
        }
