...
export function prepareMessages(
  ...
  if (lastLevel >= 2) {
    try {
      ...
      const prunedRoleSeq = pruned.map((m: any) => m.role)
      const origRoleSeq = result.map(m => m.role)
      if (prunedRoleSeq.length < origRoleSeq.length) {
        const kept: ChatMessage[] = []
        let pi = 0
        for (let oi = 0; oi < result.length && pi < prunedRoleSeq.length; oi++) {
          if (origRoleSeq[oi] === prunedRoleSeq[pi]) {
            kept.push(result[oi])
            pi++
          }
        }
        while (pi < prunedRoleSeq.length) {
          kept.push(result[result.length - prunedRoleSeq.length + pi])
          pi++
        }
        removedMessages = beforeCount - kept.length
