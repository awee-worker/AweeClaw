    ...
    try {
      ...
      const aiMessages = result.map(m => {
        if (m.role === 'assistant') {
          ...
          const hasReasoning = !!(am as any).reasoning
          return {
            role: 'assistant' as const,
            content: am.content || (hasReasoning ? ' ' : ''),
      ...
      // pruneMessages with emptyMessages:'keep' only modifies content, doesn't remove messages.
      // Check if any tool calls were pruned by comparing tool_calls presence.
      let prunedToolCallCount = 0
      for (const m of pruned) {
        if (m.role === 'assistant' && (m as any).tool_calls) {
          prunedToolCallCount += (m as any).tool_calls.length
        }
      }
      let origToolCallCount = 0
      for (const m of aiMessages) {
        if (m.role === 'assistant' && (m as any).tool_calls) {
          origToolCallCount += (m as any).tool_calls.length
        }
      }
      if (prunedToolCallCount < origToolCallCount) {
        logger.agent.info(`[Compression] pruneMessages pruned ${origToolCallCount - prunedToolCallCount} tool calls`)
      }
    } catch (e) {
