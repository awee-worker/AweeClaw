...
function tryRepairTruncatedJson(argsString: string): Record<string, unknown> | null {
  ...
  const PRIORITY_KEYS = new Set(['path', 'command', 'query', 'pattern', 'url', 'cwd', 'terminal_id'])
  ...
  if (!PRIORITY_KEYS.intersection(new Set(Object.keys(result))).size) {
    const pathMatch = /"path"\s*:\s*"((?:[^"\\]|\\.)*)"/.exec(s)
    if (pathMatch) {
      try {
        result['path'] = JSON.parse(`"${pathMatch[1]}"`)
      } catch {
        result['path'] = decodeJsonStringFragment(pathMatch[1])
      }
    }
  }
