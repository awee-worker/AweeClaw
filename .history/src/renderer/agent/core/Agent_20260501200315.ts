...
import { createSpan, endSpan, addSpanEvent } from '../harness/observability/Trace'
  ...
  async send(
    ...
    try {
      if (agentHarness.isInitialized) {
        harnessSpan = createSpan('agent.send', `thread-${threadId ?? 'new'}`, undefined, { chatMode, workspacePath })
        addSpanEvent(harnessSpan, 'execution_prepared', { threadId })
      }
      suspendAgentStorageWrites()
      ...
      if (harnessSpan) {
        addSpanEvent(harnessSpan, 'error', { message: error instanceof Error ? error.message : String(error) })
      }
      ...
      if (harnessSpan) {
        endSpan(harnessSpan)
        agentHarness.observability.trace.addSpan(harnessSpan)
      }
      if (persistSuspended) {
        resumeAgentStorageWrites()
