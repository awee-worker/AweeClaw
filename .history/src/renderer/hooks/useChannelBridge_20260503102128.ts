...
export function useChannelBridge() {
  ...
  const channelConfigsCache = useRef<ChannelConfig[]>([])
  ...
  const resolveAccountLLMConfig = useCallback(async (channelId: string, accountId: string) => {
    try {
      const res = await api.channel.getAllConfigs()
      const configs: ChannelConfig[] = res?.configs || []
      channelConfigsCache.current = configs
