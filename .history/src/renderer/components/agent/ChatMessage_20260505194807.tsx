...
const MarkdownContent = React.memo(({ content: rawContent, fontSize, isStreaming }: { content: string; fontSize: number; isStreaming?: boolean }) => {
  ...
  // 平滑流式插入
  const { displayedContent: smoothContent, isCatchingUp } = useSmoothStream(contentWithoutAlert || '', !!isStreaming, 1.5)
      ...
      {contentWithoutAlert && (
        ...
        >
          ...
          {isStreaming && smoothContent && (
            <span className={`typing-cursor ${isCatchingUp ? 'typing-cursor-solid' : ''}`} />
          )}
        </div>
      )}
    </>
...
const ThinkingIndicator = React.memo(({ waitPhase, streamStartTime, retryAttempt, retryDelay: _retryDelay }: {
  waitPhase?: string
  ...
  const phaseLabel = useMemo(() => {
    if (retryAttempt && retryAttempt > 0) {
      return t('waitPhase.retrying', language as any, { attempt: retryAttempt })
    }
    switch (waitPhase) {
  ...
  }, [waitPhase, language, retryAttempt])
...
const ChatMessage = React.memo(({
  ...
  const { isStreaming, previewMap, liveParts, liveInteractive, waitPhase, streamStartTime, retryAttempt, retryDelay, iterationIndex, iterationTotal } = useAgentStore(useShallow(state => {
    ...
    return {
      ...
      iterationIndex: isActiveAssistant ? threadStreamState?.iterationIndex : undefined,
      iterationTotal: isActiveAssistant ? threadStreamState?.iterationTotal : undefined,
    }
  }))
          ...
          <div className="w-full min-w-0 flex flex-col gap-2">
            <div className="flex items-center gap-3 px-1">
              ...
              <div className="flex items-center gap-2 select-none overflow-hidden pr-2">
                ...
                {isStreaming && retryAttempt && retryAttempt > 0 && (
                  <span className="text-[11px] text-amber-400/80 font-medium animate-breathe">
                    {t('waitPhase.retrying', language as any, { attempt: retryAttempt })}
                  </span>
                )}
              </div>
            </div>

            {isStreaming && iterationIndex && iterationIndex > 1 && (
              <div className="mx-1 mt-1 flex items-center gap-2">
                <div className="flex-1 h-[2px] rounded-full bg-accent/10 overflow-hidden">
                  <div className="h-full bg-accent/40 rounded-full transition-all duration-500 ease-out" style={{ width: `${Math.min(100, (iterationIndex / (iterationTotal || 10)) * 100)}%` }} />
                </div>
                <span className="text-[10px] text-text-muted/50 tabular-nums flex-shrink-0">#{iterationIndex}</span>
              </div>
            )}
            ...
            <div className="w-full text-[15px] leading-relaxed text-text-primary/90 pl-1">
              ...
              <div className="prose-custom w-full max-w-none">
                ...
                {isStreaming && (!assistantParts || assistantParts.length === 0) && previewToolCalls.length === 0 && (
                  <ThinkingIndicator waitPhase={waitPhase} streamStartTime={streamStartTime} retryAttempt={retryAttempt} retryDelay={retryDelay} />
                )}
            ...
            {!message.isStreaming && (
              <div className="flex items-center gap-1 pl-1 opacity-0 group-hover/msg:opacity-100 transition-opacity duration-200">
                {(() => {
                  ...
                  return (
                    <Tooltip content={language === 'zh' ? `总耗时 ${display}s` : `${display}s total`}>
                ...
                {onRegenerate && (
                  <Tooltip content={language === 'zh' ? '重新生成' : 'Regenerate'}>
                    <button
