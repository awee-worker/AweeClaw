...
const ThinkingIndicator = React.memo(({ waitPhase, streamStartTime }: {
  waitPhase?: string
  ...
  return (
    <div className="flex items-center gap-2.5 py-2 px-1">
      <div className="relative flex items-center justify-center w-5 h-5">
        <span className="absolute w-3 h-3 rounded-full bg-accent/20 animate-breathe" />
        <span className="w-1.5 h-1.5 rounded-full bg-accent/80" />
      </div>
      <span className="text-[12px] text-text-muted/80 font-medium">
        {phaseLabel}{elapsedText}
      </span>
      <div className="flex items-center gap-0.5 ml-0.5">
        <span className="wait-dot w-0.5 h-0.5 rounded-full bg-accent/60" />
        <span className="wait-dot w-0.5 h-0.5 rounded-full bg-accent/60" />
        <span className="wait-dot w-0.5 h-0.5 rounded-full bg-accent/60" />
      </div>
    </div>
...
const ChatMessage = React.memo(({
  ...
  const { isStreaming, previewMap, liveParts, liveInteractive, waitPhase, streamStartTime } = useAgentStore(useShallow(state => {
            ...
            <div className="w-full text-[15px] leading-relaxed text-text-primary/90 pl-1">
              ...
              <div className="prose-custom w-full max-w-none">
                ...
                {isStreaming && (!assistantParts || assistantParts.length === 0) && previewToolCalls.length === 0 && (
                  <ThinkingIndicator waitPhase={waitPhase} streamStartTime={streamStartTime} />
                )}
