...
const StreamingPhaseIndicator = React.memo(({
  ...
  const label = useMemo(() => {
    ...
    switch (streamDetail) {
      case 'reasoning': return t('statusBar.thinking', language as any)
  ...
  }, [mode, waitPhase, streamDetail, language, retryAttempt, isRetrying])
...
const ChatMessage = React.memo(({
  ...
  const { isStreaming, previewMap, liveParts, liveInteractive, waitPhase, streamStartTime, retryAttempt, retryDelay, iterationIndex, iterationTotal, streamDetail } = useAgentStore(useShallow(state => {
            ...
            <div className="w-full text-[15px] leading-relaxed text-text-primary/90 pl-1">
              ...
              <div className="prose-custom w-full max-w-none">
                ...
                {isStreaming && assistantParts && assistantParts.length > 0 && (
                  <StreamingPhaseIndicator mode="inline" streamDetail={streamDetail} retryAttempt={retryAttempt} hasReasoningBlock={assistantParts.some(isReasoningPart)} />
                )}
                {isStreaming && (!assistantParts || assistantParts.length === 0) && previewToolCalls.length === 0 && (
                  <StreamingPhaseIndicator mode="waiting" waitPhase={waitPhase} streamStartTime={streamStartTime} streamDetail={streamDetail} retryAttempt={retryAttempt} />
                )}
