...
export function NotesView() {
    ...
    const [notes, setNotes] = useState<Note[]>([
        { id: 'note-1', title: language === 'zh' ? '欢迎使用笔记' : 'Welcome to Notes', content: language === 'zh' ? '在这里快速记录你的想法和灵感...' : 'Quickly capture your thoughts and ideas here...', pinned: true, starred: false, createdAt: Date.now(), updatedAt: Date.now() },
    ])
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-2">
                {pinnedNotes.length > 0 && (
                    <div className="mb-2">
                        <div className="flex items-center gap-1.5 px-2 py-1">
                            ...
                            <span className="text-[10px] font-medium text-text-muted uppercase tracking-wider">{language === 'zh' ? '已置顶' : 'Pinned'}</span>
                        </div>
                    ...
                    <div>
                        {pinnedNotes.length > 0 && (
                            <div className="flex items-center gap-1.5 px-2 py-1">
                                ...
                                <span className="text-[10px] font-medium text-text-muted uppercase tracking-wider">{language === 'zh' ? '全部笔记' : 'All Notes'}</span>
                            </div>
                        )}
                        {otherNotes.map(note => (
                            <NoteItem
                ...
                {filteredNotes.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-12 text-text-muted">
                        ...
                        <p className="text-xs">{language === 'zh' ? '暂无笔记' : 'No notes yet'}</p>
                        <button onClick={handleAdd} className="mt-2 text-[11px] text-accent hover:underline">
                            {language === 'zh' ? '创建第一条笔记' : 'Create your first note'}
                        </button>
                    </div>
                )}
            </div>
                    ...
                    {isEditing ? (
                        <div className="p-3 space-y-2">
                            ...
                            <div className="flex gap-1.5 justify-end">
                                ...
                                <button onClick={handleSave} className="px-2 py-1 text-[11px] text-accent hover:text-accent/80 flex items-center gap-1"><Check className="w-3 h-3" />{language === 'zh' ? '保存' : 'Save'}</button>
                            </div>
                        </div>
                    ) : (
                        <div className="p-3">
                            <div className="flex items-center justify-between mb-1">
                                <span className="text-xs font-medium text-text-primary truncate">{selectedNote.title || (language === 'zh' ? '无标题' : 'Untitled')}</span>
                            ...
                            <p className="text-[11px] text-text-muted leading-relaxed line-clamp-4">{selectedNote.content || (language === 'zh' ? '点击编辑添加内容' : 'Click edit to add content')}</p>
                        </div>
                    )}
                </div>
            )}
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1">
                    <span className="text-[11px] font-medium truncate">{note.title || (language === 'zh' ? '无标题' : 'Untitled')}</span>
                    {note.starred && <Star className="w-2.5 h-2.5 text-amber-400 fill-amber-400 flex-shrink-0" />}
                </div>
                <p className="text-[10px] text-text-muted truncate">{note.content || (language === 'zh' ? '空笔记' : 'Empty note')}</p>
            </div>
