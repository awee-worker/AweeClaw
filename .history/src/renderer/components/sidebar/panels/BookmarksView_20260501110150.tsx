...
import {
    ...
    Star, ExternalLink, FolderOpen, FileText, Code2, Globe,
} from 'lucide-react'
                ...
                {starredBookmarks.length > 0 && (
                    <div className="mb-2">
                        ...
                        {starredBookmarks.map(bm => (
                            <BookmarkRow key={bm.id} bookmark={bm} onDelete={handleDelete} onToggleStar={handleToggleStar} language={language} />
                        ))}
                    </div>
                )}
...
function BookmarkRow({ bookmark, onDelete, onToggleStar, language }: {
