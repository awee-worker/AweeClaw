...
export function ChannelSettings({ language }: ChannelSettingsProps) {
  const raw = window.electronAPI as any
  ...
  const loadData = useCallback(async () => {
    try {
      const channelsRes = await raw.channelGetRegisteredChannels()
      const configsRes = await raw.channelGetAllConfigs()
      const statusesRes = await raw.channelGetAllAccountStatuses()
      ...
      for (const ch of channelsRes?.channels || []) {
        const res = await raw.channelGetSecretSchema(ch.id)
  ...
  const handleConnect = async (channelId: ChannelId, accountId: string) => {
    ...
    try {
      await raw.channelConnectAccount(channelId, accountId)
  ...
  const handleDisconnect = async (channelId: ChannelId, accountId: string) => {
    setActionLoading(`${channelId}:${accountId}`)
    try {
      await raw.channelDisconnectAccount(channelId, accountId)
  ...
  const handleRemoveAccount = async (channelId: ChannelId, accountId: string) => {
    try {
      await raw.channelRemoveAccount(channelId, accountId)
  ...
  const handleAddAccount = async () => {
    ...
    try {
      await raw.channelAddAccount(showAddAccount, account)
  ...
  const handleValidateCredentials = async (channelId: ChannelId) => {
    try {
      const result = await raw.channelValidateCredentials(channelId, newAccountForm.credentials)
      if (result?.valid) {
  ...
  const getAccountStatus = (channelId: ChannelId, accountId: string): ChannelAccountSnapshot | undefined => {
