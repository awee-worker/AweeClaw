...
setExternalScenarioLoadFunctions(
  ...
  async (scenarioId: string) => {
    try {
      const result = await api.scenarioInstall.readScenarioConfig(scenarioId)
      return { success: result.success, error: result.error, config: result.config as any }
    } catch (err) {
      return { success: false, error: err instanceof Error ? err.message : String(err), config: undefined }
    }
  },
)
          ...
          <>
            <div className={`flex-1 flex overflow-hidden transition-opacity duration-300 ${layoutAnimating ? 'opacity-0' : 'opacity-100'}`}>
              {layoutConfig.chatPosition === 'primary' ? (
                <>
                  {layoutConfig.showActivityBar && <ActivityBar />}
            ...
            {layoutConfig.showStatusBar && <StatusBar />}
          </>
        ) : (
