...
describe('LoopDetector', () => {
  ...
  describe('content cycle detection', () => {
    it('detects when file content oscillates between values', () => {
      ...
      for (let i = 0; i < 3; i++) {
        contents.set(path, i % 2 === 0 ? 'option-a' : 'option-b')
        const calls = [makeToolCall('edit_file', { path, content: contents.get(path)! })]
        detector.checkLoop(calls, contents)
        detector.recordExecutedTool(calls[0], true, contents)
        detector.updateContentHash(path, contents.get(path)!)
      }
