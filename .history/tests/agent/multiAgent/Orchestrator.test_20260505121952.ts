...
import type { AgentRole, CollaborationConfig } from '@shared/types/multiAgent'
...
describe('MessageBus', () => {
  ...
  it('delivers broadcast messages', () => {
    let count = 0
    bus.on('agent-1', () => { count++ })
    bus.on('agent-2', () => { count++ })
    bus.send({
      id: 'msg-1',
      from: 'orchestrator',
      to: 'broadcast',
...
describe('Orchestrator', () => {
  ...
  describe('review pattern', () => {
    it('executes review pattern and approves when reviewer approves', async () => {
      ...
      const executor = vi.fn(async (agentId: string, role: AgentRole, task: string) => {
