/**
 * Provider 健康监控 — LLM 服务商可用性检测与模型测试
 *
 * 职责：
 * - 暴露 Provider 健康检查、模型测试等 IPC 接口
 * - 通过 `IpcChannelGuard` 进行速率限制，避免频繁探测
 * - 支持场景级可用性查询（不同场景可用的 Provider 集合）
 */
...
import { IpcChannelGuard } from '../core/ipcGuard'
