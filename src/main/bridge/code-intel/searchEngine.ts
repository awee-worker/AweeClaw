/**
 * 代码搜索引擎 — 基于 ripgrep 的高性能文件搜索
 *
 * 职责：
 * - 通过 `safeIpcHandle` 注册搜索相关 IPC handler
 * - 使用 ripgrep 进行多根目录并行搜索
 * - 支持场景化搜索配置（不同场景有不同的 glob / 优先扩展名）
 * - 支持流式结果返回，降低首结果延迟
 * - 维护搜索历史记录，供 UI 层展示最近搜索
 *
 * 差异化特性（相比基础实现）：
 * - 场景搜索配置（SCENARIO_SEARCH_PROFILES）
 * - 更完整的排除模式（覆盖 node_modules / .git / dist / build / .venv 等）
 * - 流式批处理（STREAM_BATCH_SIZE / STREAM_FLUSH_INTERVAL_MS）
 * - 搜索结果上限保护（MAX_RESULTS_PER_ROOT / MAX_FILE_SIZE_BYTES）
 * - 搜索历史记录（searchHistory）
 */
