...
const COPY = {
  zh: {
    ...
    savingDescription: '请稍候，AweeClaw 会在保存完成后继续退出流程。',
    doneTitle: '保存完成',
    doneMessage: '当前工作区与对话数据已安全保存。',
    doneDescription: 'AweeClaw 正在完成最后的退出收尾。',
    errorTitle: '保存未完全成功',
    errorMessage: '部分运行时数据可能未完整持久化。',
    errorDescription: 'AweeClaw 正在继续退出，请下次启动后确认恢复结果。',
  },
  en: {
    ...
    savingDescription: 'Please wait while AweeClaw completes the shutdown flow.',
    doneTitle: 'Save complete',
    doneMessage: 'The current workspace and conversation data have been saved safely.',
    doneDescription: 'AweeClaw is finishing the last shutdown steps.',
    errorTitle: 'Save completed with warnings',
    errorMessage: 'Some runtime data may not have been fully persisted.',
    errorDescription: 'AweeClaw will keep exiting. Please verify recovery after the next launch.',
  },
} as const
