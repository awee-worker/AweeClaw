...
export class EmbeddingService {
  ...
  private async embedLLM(texts: string[]): Promise<number[][]> {
    ...
    const { EmbeddingService: LLMEmbeddingService } = await import('../modules/ai-provider/services/VectorEmbeddingEngine')
    ...
    if (!EmbeddingService.transformersPipeline) {
      if (!EmbeddingService.loadingPromise) {
        EmbeddingService.loadingPromise = (async () => {
          logger.index.info('[EmbeddingService] ProgressIndicator local transformers model:', model)
          try {
            // @ts-ignore
