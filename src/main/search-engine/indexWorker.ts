...
import { ChunkerService } from './codeChunker'
import { TreeSitterChunker } from './treeSitterParser'
import { EmbeddingService } from './vectorEmbedder'
import { CodeChunk, IndexedChunk, IndexConfig } from './providerTypes'
...
async function handleIndex(
  ...
  if (config.mode === 'semantic' && config.embedding.provider === 'transformers') {
    postResponse({ type: 'progress', processed: 0, total: totalFiles, message: 'ProgressIndicator embedding model (first run may take longer)...' })
    try {
