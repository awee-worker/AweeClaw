...
// ==========================================
// Chromium 命令行开关（必须在 app ready 之前设置）
// ==========================================

/**
 * 忽略 GPU 黑名单，强制走硬件加速。
 *
 * 背景：透明窗口 + WebGL（桌面伴侣）在软件渲染（SwiftShader）路径下，
 * 半透明像素的合成结果与硬件路径不一致 —— 打包安装后的表现是画面发白、
 * 失去色彩（像底片），而开发态不明显。参考实现 example/super-ai-browser
 * 同样使用了该开关，这里对齐以保证打包后仍走 GPU 合成。
 */
