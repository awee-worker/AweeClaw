...
export type ToolCategory = 'read' | 'write' | 'terminal' | 'search' | 'lsp' | 'network' | 'interaction' | 'plan' | 'data' | 'media' | 'office' | 'desktop'
...
- The file will be sent to the same conversation where the user sent the message`,
    ...
    desktop_list_apps: {
        ...
        description: 'List installed desktop applications. Call this FIRST when you need to launch or quit an app but are unsure of its exact registered name. Returns app names, bundle IDs, and paths.',
        detailedDescription: `List installed desktop applications on the system.

- Returns a list of installed apps with their names, bundle IDs (macOS), executable paths, versions, and publishers
- Use this tool BEFORE desktop_launch_app or desktop_quit_app when the exact app name is unknown
- Supports optional filter parameter to narrow results by name keyword
- Results are cached after first scan for performance; subsequent calls are fast
- Typical usage: user says "open WeChat" → call desktop_list_apps(filter="wechat") → find exact name → call desktop_launch_app(name=exactName)`,
        category: 'desktop',
        approvalType: 'none',
        parallel: true,
        requiresWorkspace: false,
        enabled: true,
        parameters: {
            filter: { type: 'string', description: 'Optional keyword to filter apps by name (case-insensitive). E.g., "wechat", "code", "chrome". If omitted, returns all installed apps.', required: false },
        },
    },

    desktop_launch_app: {
        name: 'desktop_launch_app',
        displayName: 'Launch Desktop App',
        description: 'Launch a desktop application by name. IMPORTANT: If unsure of the exact app name, call desktop_list_apps first to get the list of installed apps. App name must match the system registered name exactly.',
        detailedDescription: `Launch a desktop application.

- App name must match the system registered name exactly (e.g., "Safari", "Calculator", "Visual Studio Code")
- If unsure of the exact name, call desktop_list_apps first to query installed apps
- Optional args can be passed for apps that support them
- On macOS, uses NSWorkspace; on Windows, uses shell; on Linux, uses xdg-open
- If launch fails, the error message will include similar app names as suggestions`,
        category: 'desktop',
        approvalType: 'interaction',
        parallel: false,
        requiresWorkspace: false,
        enabled: true,
        parameters: {
            name: { type: 'string', description: 'Application name (must match system registered name, e.g., "Safari", "Calculator", "Visual Studio Code"). Call desktop_list_apps if unsure.', required: true },
            args: { type: 'array', description: 'Optional command-line arguments', items: { type: 'string', description: 'Argument string' } },
        },
    },

    desktop_quit_app: {
        name: 'desktop_quit_app',
        displayName: 'Quit Desktop App',
        description: 'Quit a running desktop application by name. If unsure of the exact app name, call desktop_list_apps first. Gracefully sends a quit signal to the application.',
        category: 'desktop',
        approvalType: 'interaction',
        parallel: false,
        requiresWorkspace: false,
        enabled: true,
        parameters: {
            name: { type: 'string', description: 'Application name to quit (must match system registered name). Call desktop_list_apps if unsure.', required: true },
        },
    },

    desktop_list_windows: {
        name: 'desktop_list_windows',
        displayName: 'List Desktop Windows',
        description: 'List all visible desktop windows with their titles, positions, and sizes. Useful for finding a specific window to focus or interact with.',
        category: 'desktop',
        approvalType: 'none',
        parallel: true,
        requiresWorkspace: false,
    ...
    desktop_focus_window: {
        ...
        description: 'Bring a specific window to the front and give it focus. Use desktop_list_windows first to get the window ID.',
        category: 'desktop',
        approvalType: 'none',
        parallel: false,
        requiresWorkspace: false,
        enabled: true,
        parameters: {
            window_id: { type: 'string', description: 'Window identifier from desktop_list_windows', required: true },
        },
    },
    ...
    desktop_close_window: {
        ...
        description: 'Close a specific window by its ID. Use desktop_list_windows first to find the window.',
        category: 'desktop',
        approvalType: 'interaction',
        parallel: false,
        requiresWorkspace: false,
    ...
    desktop_capture_screen: {
        ...
        description: 'Capture a screenshot of the entire screen or a specific display. Returns a base64-encoded PNG image. Use this for visual analysis of the desktop.',
        detailedDescription: `Capture the screen for visual analysis.
- Without display_id, captures the primary display
- Returns base64-encoded PNG image data
- On macOS, may require Screen Recording permission`,
        category: 'desktop',
        approvalType: 'none',
        parallel: true,
        requiresWorkspace: false,
        enabled: true,
        parameters: {
            display_id: { type: 'number', description: 'Display index (0 = primary). Omit for primary display.' },
        },
    },
    ...
    desktop_mouse_click: {
        ...
        description: 'Simulate a mouse click at the specified screen coordinates. Coordinates are in screen pixels with origin at top-left.',
        detailedDescription: `Simulate a mouse click.
- Coordinates are screen pixels, origin at top-left
- button: "left" (default), "right", or "middle"
- click_type: "single" (default) or "double"
- macOS requires Accessibility permission`,
        ...
        enabled: true,
        parameters: {
            x: { type: 'number', description: 'X coordinate (screen pixels)', required: true },
            y: { type: 'number', description: 'Y coordinate (screen pixels)', required: true },
            button: { type: 'string', description: 'Mouse button', enum: ['left', 'right', 'middle'] },
            click_type: { type: 'string', description: 'Click type', enum: ['single', 'double'] },
        },
    },
    ...
    desktop_mouse_move: {
        ...
        parameters: {
            ...
            smooth: { type: 'boolean', description: 'Whether to animate the movement' },
            duration: { type: 'number', description: 'Animation duration in ms (if smooth)' },
        },
    },
    ...
    desktop_mouse_scroll: {
        ...
        description: 'Simulate mouse scroll wheel at the specified coordinates. Positive amount scrolls up, negative scrolls down.',
        category: 'desktop',
        approvalType: 'none',
        parallel: false,
        requiresWorkspace: false,
        ...
        parameters: {
            ...
            amount: { type: 'number', description: 'Scroll amount (positive=up, negative=down)', required: true },
        },
    },
    ...
    desktop_type_text: {
        ...
        description: 'Type text string at the current cursor position. Simulates keyboard input character by character.',
        detailedDescription: `Type text at current cursor position.
- Text is typed at wherever the cursor is currently focused
- Optional delay between keystrokes for compatibility
- macOS requires Accessibility permission`,
        category: 'desktop',
        approvalType: 'interaction',
        parallel: false,
    ...
    desktop_press_key: {
        ...
        description: 'Press and release a single keyboard key. Use key names like "enter", "tab", "escape", "space", "up", "down", "left", "right", "backspace", "delete".',
        category: 'desktop',
        approvalType: 'interaction',
        parallel: false,
        requiresWorkspace: false,
        enabled: true,
        parameters: {
            key: { type: 'string', description: 'Key name (e.g., "enter", "tab", "escape", "space", "up")', required: true },
        },
    },
    ...
    desktop_key_combo: {
        ...
        description: 'Press a keyboard key combination (e.g., Cmd+C, Ctrl+Shift+Esc). Keys are pressed simultaneously.',
        detailedDescription: `Press a key combination.
- Keys are pressed together (chord)
- Use platform-appropriate modifiers: "cmd" (macOS), "ctrl" (Windows/Linux)
- Example: ["cmd", "c"] for copy, ["ctrl", "shift", "esc"] for Task Manager`,
        category: 'desktop',
        approvalType: 'interaction',
        parallel: false,
        requiresWorkspace: false,
        ...
        parameters: {
            keys: {
                type: 'array',
                description: 'Array of key names to press together (e.g., ["cmd", "c"])',
                required: true,
                items: { type: 'string', description: 'Key name (e.g., "cmd", "ctrl", "shift", "alt", "c")' },
            },
        },
    },
    ...
    desktop_emergency_stop: {
        ...
        description: 'Immediately halt all desktop automation operations. Use this when something goes wrong or the user asks to stop. After triggering, all desktop control operations will be rejected until manually reset.',
        detailedDescription: `Trigger emergency stop for desktop control.
- Immediately aborts all ongoing desktop operations
- Blocks all future desktop operations until reset
- Use this when:
  - The user says "stop" or "cancel"
  - An operation is behaving unexpectedly
  - You detect a potentially harmful situation
- After stopping, the user must manually reset via the UI`,
        category: 'desktop',
        approvalType: 'none',
        parallel: false,
        requiresWorkspace: false,
    ...
    desktop_record_action: {
        ...
        description: 'Record a single desktop action into the current recording session. Use this to capture user operations for later replay. Must call desktop_recording_start first.',
        detailedDescription: `Record a desktop action into the active recording session.
- Requires an active recording session (call desktop_recording_start first)
- Supported action types: mouse_click, mouse_move, mouse_scroll, key_press, key_combo, type_text, window_focus, app_launch, wait
- Each action is timestamped relative to recording start
- Consecutive mouse moves and text inputs are automatically compressed
- Use desktop_recording_stop to finalize and save the recording`,
        category: 'desktop',
        approvalType: 'interaction',
        parallel: false,
        requiresWorkspace: false,
        ...
        parameters: {
            actionType: {
                type: 'string',
                description: 'Type of action to record: mouse_click, mouse_move, mouse_scroll, key_press, key_combo, type_text, window_focus, app_launch, wait',
            },
            params: {
                type: 'object',
                description: 'Action parameters (varies by actionType)',
            },
        },
    },
    ...
    desktop_recording_start: {
        ...
        description: 'Start a new desktop operation recording session. Only one session can be active at a time. Call desktop_recording_stop to finalize.',
        detailedDescription: `Start recording desktop operations.
- Creates a new recording session with a unique ID
- Only one session can be active at a time
- Use desktop_record_action to add events to the recording
- Use desktop_recording_stop to finalize and get the script
- The recording can be replayed later with desktop_replay_recording`,
        category: 'desktop',
        approvalType: 'interaction',
        parallel: false,
        requiresWorkspace: false,
    ...
    desktop_recording_stop: {
        ...
        description: 'Stop the current recording session and save the script. Returns the recording ID for later replay.',
        detailedDescription: `Stop the active recording session.
- Flushes any pending buffered events
- Returns the recording script with all recorded events
- The script is saved and can be replayed with desktop_replay_recording
- If discard=true, the recording is discarded without saving`,
        category: 'desktop',
        approvalType: 'interaction',
        parallel: false,
        requiresWorkspace: false,
        enabled: true,
        parameters: {
            discard: { type: 'boolean', description: 'If true, discard the recording without saving' },
        },
    },

    desktop_replay_recording: {
        name: 'desktop_replay_recording',
        displayName: 'Replay Recording',
        description: 'Replay a previously recorded desktop operation script. The recording will be executed with configurable speed and error handling.',
        detailedDescription: `Replay a recorded desktop operation script.
- Requires a valid recording ID (from desktop_recording_stop)
- Supports multiple replay speeds: realtime, fast, instant, custom
- Can be aborted via emergency stop
- Returns execution statistics (success/error counts, duration)
- Use desktop_list_recordings to see available recordings`,
        category: 'desktop',
        approvalType: 'dangerous',
        parallel: false,
        requiresWorkspace: false,
        enabled: true,
        parameters: {
            recordingId: { type: 'string', description: 'ID of the recording to replay' },
            speed: {
                type: 'string',
                description: 'Replay speed: realtime, fast, instant, custom',
            },
            speedMultiplier: { type: 'number', description: 'Custom speed multiplier (speed=custom only, 1.0=realtime)' },
            stopOnError: { type: 'boolean', description: 'Stop replay on first error' },
        },
    },

    desktop_list_recordings: {
        name: 'desktop_list_recordings',
        displayName: 'List Recordings',
        description: 'List all saved desktop operation recordings. Returns metadata for each recording including name, duration, and event count.',
        detailedDescription: `List all saved desktop operation recordings.
- Returns an array of recording metadata
- Each entry includes: id, name, description, duration, eventCount, createdAt
- Use desktop_replay_recording with the ID to replay a recording`,
        category: 'desktop',
        approvalType: 'none',
        parallel: false,
        requiresWorkspace: false,
    ...
    desktop_visual_agent_step: {
        name: 'desktop_visual_agent_step',
        displayName: 'Visual Agent Step',
        description: 'Execute a visual feedback loop step: capture screen, analyze with vision model, decide and execute next action. Use for GUI tasks that require visual understanding.',
        detailedDescription: `Run a visual feedback loop for GUI automation.
- Captures a screenshot of the current screen
- Sends it to a vision-capable LLM for analysis
- The LLM decides the next action (click, type, scroll, etc.)
- Executes the action and returns the result
- Set maxSteps to limit iterations (default 10, max 50)
- The loop stops when:
  - The LLM signals task completion
  - Max steps reached
  - Emergency stop triggered
  - An unrecoverable error occurs
- Requires a vision-capable model configuration`,
        category: 'desktop',
        approvalType: 'dangerous',
        parallel: false,
        requiresWorkspace: false,
        enabled: true,
        parameters: {
            task: { type: 'string', description: 'Natural language description of the task to accomplish' },
            maxSteps: {
                type: 'number',
                description: 'Maximum number of loop iterations (1-50, default 10)',
            },
        },
    },

    desktop_workflow_run: {
        name: 'desktop_workflow_run',
        displayName: 'Run Workflow',
        description: 'Execute a predefined desktop automation workflow. Workflows can chain multiple steps with conditions, loops, and error handling.',
        detailedDescription: `Run a predefined desktop automation workflow.
- Executes a sequence of steps: desktop actions, recordings, visual loops, conditions
- Supports variables, conditionals, loops, and error handling
- Can be triggered manually or via cron/event
- Returns execution result with step-by-step logs
- Use desktop_workflow_list to see available workflows`,
        category: 'desktop',
        approvalType: 'dangerous',
        parallel: false,
        requiresWorkspace: false,
        ...
        parameters: {
            ...
            variables: {
                type: 'object',
                description: 'Input variables to override workflow defaults',
            },
        },
    },

    desktop_workflow_list: {
        name: 'desktop_workflow_list',
        displayName: 'List Workflows',
        description: 'List all registered desktop automation workflows. Returns metadata for each workflow including name, triggers, and step count.',
        detailedDescription: `List all registered desktop automation workflows.
- Returns an array of workflow metadata
- Each entry includes: id, name, description, triggers, stepCount, enabled
- Use desktop_workflow_run with the ID to execute a workflow`,
        category: 'desktop',
        approvalType: 'none',
        parallel: false,
        requiresWorkspace: false,
...
export function generateToolsPromptDescriptionFiltered(
    ...
    if (categories.desktop.length > 0) {
        sections.push('## Desktop Control Tools')
        sections.push(DESKTOP_TOOLS_GUIDE)
        for (const config of categories.desktop) {
            sections.push(generateToolPromptDescription(config))
        }
    }
