...
class WorkflowEngineClass {
  ...
  private async handleError(
    ...
    switch (step.onError.action) {
      ...
      case 'abort':
        ...
        break
      case 'retry': {
        const maxRetries = step.onError!.maxRetries ?? 0
        const retryKey = `${run.id}:${step.id}`
        const currentRetries = this.retryCounters.get(retryKey) ?? 0
        if (currentRetries < maxRetries) {
          this.retryCounters.set(retryKey, currentRetries + 1)
          if (step.onError!.retryDelayMs) {
            await new Promise(resolve => setTimeout(resolve, step.onError!.retryDelayMs))
          }
          const lastResult = run.stepHistory[run.stepHistory.length - 1]
          if (lastResult?.snapshot) {
            this.restoreSnapshot(run, lastResult.snapshot)
          }
          await this.executeStep(run, step.id, definition)
        } else {
          this.retryCounters.delete(retryKey)
          run.status = 'failed'
          run.error = `Max retries (${maxRetries}) exceeded for step '${step.id}': ${error}`
        }
        break
      }
      case 'goto':
  ...
  rollbackStep(runId: string, stepIndex: number): boolean {
    const run = this.runs.get(runId)
    if (!run) return false
    if (stepIndex < 0 || stepIndex >= run.stepHistory.length) return false

    const targetResult = run.stepHistory[stepIndex]
    if (!targetResult.snapshot) return false

    this.restoreSnapshot(run, targetResult.snapshot)

    for (let i = run.stepHistory.length - 1; i > stepIndex; i--) {
      run.stepHistory[i].status = 'rolled_back'
    }
