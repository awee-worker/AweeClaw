    ...
    try {
      ...
      if (cfg.abortSignal) {
        await new Promise<void>((resolve, reject) => {
          ...
          cfg.abortSignal!.addEventListener('abort', onAbort, { once: true })
        })
      } else {
...
export class RetryMiddleware {
  ...
  shouldRetry(error: unknown, attempt: number): boolean {
    return shouldRetryAfterError(error, attempt, this.maxRetries)
  }
...
export function shouldRetryAfterError(error: unknown, attempt: number, maxAttempts: number = 3): boolean {
