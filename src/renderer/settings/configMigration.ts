...
import type { SettingsState, ProviderModelConfig } from '@shared/configuration/preferenceSync'
import type { AppSettings } from '@shared/configuration/providerTypes'
...
export function downloadSettings(settings: SettingsState, includeApiKeys = false): void {
  ...
  a.href = url
  a.download = `aweeclaw-settings-${new Date().toISOString().split('T')[0]}.json`
