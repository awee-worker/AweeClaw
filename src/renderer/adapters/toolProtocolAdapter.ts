...
import { toAppError, getErrorMessage } from '@shared/toolkit/errorCatalog'
import type {
  McpServerState,
  McpTool,
  McpResource,
...
class McpService {
  ...
  private setupEventListeners(): void {
    ...
    const cleanupStatus = api.mcp.onServerStatus((event: { serverId: string; status: string; error?: string }) => {
      // 验证状态值是否有效
      const validStatuses: import('@shared/protocols/toolProtocolBridge').McpServerStatus[] = [
        'disconnected', 'connecting', 'connected', 'error', 'needs_auth', 'needs_registration'
      ]
      // 类型守卫函数
      const isValidStatus = (status: string): status is import('@shared/protocols/toolProtocolBridge').McpServerStatus => {
        return validStatuses.includes(status as import('@shared/protocols/toolProtocolBridge').McpServerStatus)
      }
      const status: import('@shared/protocols/toolProtocolBridge').McpServerStatus = isValidStatus(event.status) 
        ? event.status
        : 'error'
