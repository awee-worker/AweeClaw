...
export interface MessageActions {
    ...
    updateLintCheckPart: (messageId: string, updates: Partial<import('../../providerTypes').LintCheckPart>, targetThreadId?: string) => void
    addSystemAlertPart: (
        messageId: string,
        alert: {
        ...
        set(state => {
            ...
            const messages = thread.messages.map(msg => {
                ...
                const existing = new Set(
                    items
                        .filter((i): i is import('../../providerTypes').SkillContext => i.type === 'Skill')
                        .map(i => i.skillId)
                )
