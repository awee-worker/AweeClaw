...
export interface ILintService {
  lintFile(filePath: string): Promise<import('../../providerTypes').LintError[]>
}
...
export interface ILlmConfigService {
  getConfig(): import('@protocols/modelGateway').LLMConfig | null
  setConfig(config: Partial<import('@protocols/modelGateway').LLMConfig>): void
}
