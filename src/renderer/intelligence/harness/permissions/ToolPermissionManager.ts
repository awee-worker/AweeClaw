import type { ToolCategory } from '@shared/configuration/toolDefinitions'
...
export class ToolPermissionManager {
  ...
  private findMatchingRule(
    ...
    let bestMatch: ToolPermissionRule | undefined
    let bestScore = -1
    ...
    for (const rule of this.rules) {
      ...
      let score = 0
      if (rule.toolName) score += 10
      if (rule.category) score += 5
      if (rule.conditions && rule.conditions.length > 0) score += rule.conditions.length
