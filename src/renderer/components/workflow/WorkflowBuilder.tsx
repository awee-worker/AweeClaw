...
export default function WorkflowBuilder({ workflow, onSave, onCancel, language }: WorkflowBuilderProps) {
  ...
  const orderedSteps = useMemo(() => {
    ...
    while (currentId && !visited.has(currentId)) {
      ...
      const currentStep: WorkflowStep | undefined = wf.steps[currentId]
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const newParam: WorkflowInputParam = { type: 'string', description: '', descriptionZh: '', required: false }
                      const newSchema = { ...(wf.inputSchema || {}), [paramId]: newParam }
                ...
                {(!wf.inputSchema || Object.keys(wf.inputSchema).length === 0) ? (
                  ...
                  <div className="space-y-2">
                    {Object.entries(wf.inputSchema || {}).map(([key, param]) => (
                      <div key={key} className="p-2.5 rounded-lg border border-border/20 bg-surface/20 space-y-2">
                        <div className="flex items-center gap-2">
                          <input
                            type="text"
                            value={key}
                            onChange={e => {
                              const newSchema = { ...wf.inputSchema }
                          ...
                          <button
                            ...
                            className="p-1 text-text-muted/30 hover:text-red-400 transition-colors"
                          >
                            <Trash2 className="w-3 h-3" />
                          </button>
                        </div>
                        <input
                          type="text"
                          value={language === 'zh' ? (param.descriptionZh || param.description) : param.description}
                          onChange={e => {
                            const newSchema = { ...wf.inputSchema }
                            newSchema[key] = {
                              ...param,
                              description: e.target.value,
                              ...(language === 'zh' ? { descriptionZh: e.target.value } : {}),
                            }
                            updateWorkflow({ inputSchema: newSchema })
                          }}
                        ...
                        <div className="flex items-center gap-3">
                          <select
                            ...
                            onChange={e => {
                              const newSchema = { ...wf.inputSchema }
                              newSchema[key] = { ...param, type: e.target.value as any }
                              updateWorkflow({ inputSchema: newSchema })
                            }}
                            className="px-2 py-0.5 text-[10px] bg-surface/50 border border-border/30 rounded text-text-primary focus:outline-none"
                          >
                            <option value="string">string</option>
                            <option value="number">number</option>
                            <option value="boolean">boolean</option>
                          </select>
                          <label className="flex items-center gap-1 text-[10px] text-text-muted/50">
                            <input
                              type="checkbox"
                              checked={param.required || false}
                              onChange={e => {
                                const newSchema = { ...wf.inputSchema }
                                newSchema[key] = { ...param, required: e.target.checked }
                                updateWorkflow({ inputSchema: newSchema })
                              }}
    ...
    <div className="p-5 space-y-5">
      ...
      {role && (
        <div
          ...
          style={{ backgroundColor: `${role.color}08`, borderColor: `${role.color}30` }}
        >
          <div
            className="w-5 h-5 rounded flex items-center justify-center"
            style={{ backgroundColor: `${role.color}15` }}
          >
            <MessageSquare className="w-3 h-3" style={{ color: role.color }} />
          </div>
          <div>
            <p className="text-[10px] font-medium" style={{ color: role.color }}>
              {language === 'zh' ? role.nameZh : role.name}
            </p>
            <p className="text-[9px] text-text-muted/50">
