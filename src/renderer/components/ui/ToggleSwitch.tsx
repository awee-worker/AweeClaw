...
const SIZE_CFG: Record<string, SizeCfg> = {
  sm: { track: 'w-8 h-4', thumb: 'w-3 h-3', shift: 'peer-checked:translate-x-4', font: 'text-[9px]' },
  md: { track: 'w-11 h-6', thumb: 'w-5 h-5', shift: 'peer-checked:translate-x-5', font: 'text-xs' },
  lg: { track: 'w-14 h-7', thumb: 'w-6 h-6', shift: 'peer-checked:translate-x-7', font: 'text-sm' },
}
    ...
    return (
      <label className={`inline-flex items-center cursor-pointer group select-none ${className}`}>
        <div className="relative">
          ...
          <div className={`absolute top-0.5 left-0.5 bg-white ${cfg.thumb} rounded-full shadow-md transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] ${cfg.shift} group-hover:scale-95 peer-checked:group-hover:scale-110`}>
