...
const ModelSelector = memo(function ModelSelector({
    ...
    // 获取当前厂商的模型列表（去重）并转换为 DropdownSelector 选项
...
export const ExecutionBoard = memo(function ExecutionBoard({ planId }: TaskBoardProps) {
    ...
    useEffect(() => {
        ...
        const loadRequirements = async () => {
            try {
                const mdPath = `${workspacePath}/${BRAND.dirName}/planner/${plan.requirementsDoc}`
    ...
    const handleStart = useCallback(async () => {
        if (plan) {
            ...
            const { startPlanExecution } = await import('@intelligence/planner/taskExecutor')
    ...
    const handleStop = useCallback(async () => {
        ...
        const { stopPlanExecution } = await import('@intelligence/planner/taskExecutor')
        stopPlanExecution(planId)
    }, [planId])

    const handlePause = useCallback(async () => {
        const { pausePlanExecution } = await import('@intelligence/planner/taskExecutor')
            ...
            <div className="flex-shrink-0 p-4 border-b border-border bg-surface/30 backdrop-blur-sm">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {isExecuting ? (
                            <>
                                <ActionButton variant="secondary" size="sm" onClick={handlePause} disabled={plan.status === 'pausing' || plan.status === 'stopping'}>
                                    <Pause className="w-4 h-4 mr-1" />
                                    暂停
                                </ActionButton>
                                <ActionButton variant="danger" size="sm" onClick={handleStop} disabled={plan.status === 'stopping'}>
                            ...
                            <ActionButton variant="primary" size="sm" onClick={handleStart}>
                                <Play className="w-4 h-4 mr-1" />
                                开始执行
                            </ActionButton>
                        )}
