    ...
    <div className="bg-transparent flex flex-col transition-none relative z-10" style={{ height: isCollapsed ? 40 : height }}>
      ...
      <div className="h-10 min-h-[40px] flex items-center justify-between border-b border-border-subtle bg-background/40 backdrop-blur-md select-none">
        ...
        <div className="flex items-center gap-1">
          {isIdle ? (
            <ActionButton variant="ghost" size="icon" onClick={handleStartDebug} data-debug-start className="h-7 w-7" title="F5 - Start Debugging">
            ...
            <>
              ...
              {isPaused ? (
                <ActionButton variant="ghost" size="icon" onClick={handleContinue} className="h-7 w-7" title="F5 - Continue">
                  <Play className="w-4 h-4 text-green-400" />
                </ActionButton>
              ) : (
                <ActionButton variant="ghost" size="icon" onClick={handlePause} className="h-7 w-7" title="F6 - Pause">
                  <Pause className="w-4 h-4 text-yellow-400" />
                </ActionButton>
              )}
              <div className="w-px h-4 bg-border-subtle mx-1" />
              <ActionButton variant="ghost" size="icon" onClick={handleStepOver} disabled={!isPaused} className="h-7 w-7" title="F10 - Step Over">
                <SkipForward className="w-4 h-4" />
              </ActionButton>
              <ActionButton variant="ghost" size="icon" onClick={handleStepInto} disabled={!isPaused} className="h-7 w-7" title="F11 - Step Into">
                <ArrowDownToLine className="w-4 h-4" />
              </ActionButton>
              <ActionButton variant="ghost" size="icon" onClick={handleStepOut} disabled={!isPaused} className="h-7 w-7" title="Shift+F11 - Step Out">
