                ...
                <div className="h-[42px] min-h-[42px] flex items-center justify-between border-b border-border/50 bg-background select-none relative z-20 px-2 py-1.5">
                    ...
                    <div className="flex items-center gap-1 px-2 flex-shrink-0 h-full">
                        <div className="relative flex items-center h-full border-l border-border/50 pl-2">
                            <ActionButton ref={scriptButtonRef} variant="ghost" size="icon" onClick={() => setShowScriptMenu(!showScriptMenu)} className="h-7 w-7 rounded-lg text-green-400 hover:bg-green-500/10" title="Run Task">
                        ...
                        <ActionButton variant="ghost" size="icon" onClick={handleFixWithAI} className="h-7 w-7 rounded-lg text-accent hover:bg-accent/10" title="Fix with AI">
                            <Sparkles className="w-3.5 h-3.5" />
                        </ActionButton>
                        <div className="w-[1px] h-4 bg-border/50 mx-1" />
                        <ActionButton variant="ghost" size="icon" onClick={() => { createTerminal(); setTerminalLayout('split'); }} className="h-7 w-7 rounded-lg text-text-muted hover:text-text-primary hover:bg-surface-hover" title="Split Terminal"><SplitSquareHorizontal className="w-3.5 h-3.5" /></ActionButton>
                        <ActionButton variant="ghost" size="icon" onClick={() => activeId && terminalManager.getXterm(activeId)?.clear()} className="h-7 w-7 rounded-lg text-text-muted hover:text-text-primary hover:bg-surface-hover" title="Clear"><Trash2 className="w-3.5 h-3.5" /></ActionButton>
                        <ActionButton variant="ghost" size="icon" onClick={closePanel} className="h-7 w-7 rounded-lg text-text-muted hover:text-text-primary hover:bg-surface-hover" title="Close"><X className="w-3.5 h-3.5" /></ActionButton>
                    </div>
                </div>
                            ...
                            >
                                {isSplitView && (
                                    <div className="absolute top-0 right-0 p-1 z-10 opacity-0 group-hover/term:opacity-100 transition-opacity">
                                        <ActionButton variant="ghost" size="icon" onClick={(e) => closeTerminal(term.id, e)} className="h-6 w-6 bg-background/80 hover:bg-red-500 hover:text-text-inverted">
                                            <X className="w-3 h-3" />
                                        </ActionButton>
                                    </div>
                                )}
