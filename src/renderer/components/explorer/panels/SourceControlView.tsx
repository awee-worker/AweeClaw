...
import { TextField, ActionButton, OverlayDialog, DropdownSelector } from '@components/ui'
import { getFileName, joinPath, normalizePath, toFullPath } from '@shared/toolkit/pathHelper'
import { ConflictResolver } from '@components/version-control/ConflictResolver'
...
Commit message:`
            ...
            >
                <div className="flex items-center gap-2 border-b border-border-subtle px-3 py-2">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                        <ActionButton variant="icon" size="icon" onClick={() => refreshRepoSnapshot(repo.root)} title={tt('refresh')} className="h-6 w-6 rounded-md">
    ...
    if (!workspacePath) {
        return (
            <div className="flex h-full flex-col items-center justify-start overflow-y-auto px-6 pb-6 pt-24 text-center">
                ...
                <ActionButton onClick={handleOpenFolder} size="sm" className="px-4">
        ...
        return (
            <div className="flex h-full flex-col items-center justify-start overflow-y-auto px-6 pb-6 pt-24 text-center">
                ...
                <div className="flex w-full max-w-[260px] flex-col gap-2">
                    <ActionButton onClick={handleInit} size="sm" className="w-full">
                    ...
                    {showCloneInput && (
                        <div className="flex flex-col gap-2 rounded-lg border border-border-subtle bg-surface/40 p-2">
                            <TextField
                                ...
                                disabled={isCloning}
                            />
                            <ActionButton onClick={handleClone} size="sm" isLoading={isCloning} className="w-full">
                                {isCloning ? tt('git.cloning') : tt('git.clone')}
                            </ActionButton>
                        </div>
                    )}
                    <div className="grid grid-cols-2 gap-2">
                        <ActionButton variant="outline" size="sm" onClick={handleOpenFolder}>
                            <FolderOpen className="w-3.5 h-3.5" />
                            {tt('openFolder')}
                        </ActionButton>
                        <ActionButton variant="ghost" size="sm" onClick={handleRefreshAll}>
    ...
    if (!isRepoListMode && !status && (isRefreshing || isGitRepository === null || error)) {
        return (
            <div className="flex h-full flex-col items-center justify-start overflow-y-auto px-6 pb-6 pt-24 text-center">
                ...
                <ActionButton variant="outline" size="sm" onClick={handleRefreshAll} disabled={isRefreshing || isDiscoveringRepos}>
    ...
    return (
        <div className="flex flex-col h-full bg-transparent text-sm">
            ...
            <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-md border-b border-border-subtle flex flex-col shadow-sm">
                ...
                <div className="h-11 px-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                        ...
                        {!isRepoListMode && (
                            <>
                                <ActionButton variant="icon" size="icon" onClick={() => handleFetch()} title={tt('git.fetch')} className="w-7 h-7 rounded-lg">
                                    <ArrowDown className="w-3.5 h-3.5" />
                                </ActionButton>
                                <ActionButton variant="icon" size="icon" onClick={() => handlePull()} disabled={isPulling} title={tt('git.pull')} className="w-7 h-7 rounded-lg">
                                    <ArrowDown className={`w-3.5 h-3.5 ${isPulling ? 'animate-bounce' : ''}`} />
                                </ActionButton>
                                <ActionButton variant="icon" size="icon" onClick={() => handlePush()} disabled={isPushing} title={tt('git.push')} className="w-7 h-7 rounded-lg">
                                    <ArrowUp className={`w-3.5 h-3.5 ${isPushing ? 'animate-bounce' : ''}`} />
                                </ActionButton>
                            </>
                        )}
                        <ActionButton variant="icon" size="icon" onClick={handleRefreshAll} title={tt('refresh')} className="w-7 h-7 rounded-lg text-text-muted hover:text-text-primary">
                        ...
                        <div className="px-3 py-3 bg-surface/30 border-b border-border-subtle">
                            ...
                            {showStashInput && (
                                <div className="mt-2 flex items-center gap-2 animate-slide-in">
                                    ...
                                    />
                                    <ActionButton size="sm" onClick={handleStash} className="h-8">
                            ...
                            {showNewBranch ? (
                                <div className="flex items-center gap-2">
                                    <TextField
                                        ...
                                        autoFocus
                                    />
                                    <ActionButton size="sm" onClick={handleCreateBranch} className="h-8">
                                        <Check className="w-3 h-3" />
                                    </ActionButton>
                                    <ActionButton variant="ghost" size="sm" onClick={() => setShowNewBranch(false)} className="h-8">
                                        <X className="w-3 h-3" />
                                    </ActionButton>
                                </div>
                            ) : (
                                <ActionButton variant="secondary" onClick={() => setShowNewBranch(true)} className="w-full">
            ...
            {/* Conflict Resolver OverlayDialog */}
            {conflictFile && (
                <OverlayDialog isOpen={true} onClose={() => setConflictFile(null)} title="" size="5xl" noPadding>
                    <ConflictResolver
