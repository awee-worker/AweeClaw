...
import { ChatThread, getMessageText, getThreadDisplayTitle } from '@intelligence/providerTypes'
import { Branch } from '@intelligence/state/slices/conversationBranch'
import { ActionButton } from '../ui'
import { HintOverlay } from '../ui/HintOverlay'
import { useStore } from '@store'
import { getRelativeTime } from '@toolkit'
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-border/30 select-none">
              ...
              <ActionButton variant="ghost" size="icon" onClick={onClose} className="h-8 w-8 rounded-full hover:bg-surface/80 text-text-muted hover:text-text-primary transition-colors">
                <X className="w-4 h-4" />
              </ActionButton>
            </div>
...
function ThreadItem({ thread, isActive, isEditing, editName, language, onSelect, onDelete, onStartEdit, onEditNameChange, onSaveEdit, onCancelEdit }: {
  ...
  const timeStr = getRelativeTime(thread.lastModified, language as 'zh' | 'en')
    ...
    >
      ...
      <div className="flex-1 min-w-0 overflow-hidden pt-0.5">
        {isEditing ? (
          <div className="flex items-center gap-1 pr-6" onClick={e => e.stopPropagation()}>
            ...
            />
            <ActionButton size="icon" variant="ghost" className="h-7 w-7 text-green-400 hover:bg-green-500/10" onClick={e => {
              e.stopPropagation()
              onSaveEdit()
            }}>
        ...
        <div className="flex items-center gap-3 mt-1.5">
          ...
          <span className="text-[11px] text-text-muted/85">
            {(thread.messageCount ?? thread.messages.length)} {language === 'zh' ? '条消息' : 'msgs'}
          </span>
        </div>
      </div>
      ...
      {!isEditing && (
        <div className="absolute right-2 top-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-background/50 backdrop-blur-sm rounded-lg">
          <HintOverlay content={language === 'zh' ? '重命名' : 'Rename'}>
            <button
              onClick={e => {
                e.stopPropagation()
                onStartEdit()
          ...
          <HintOverlay content={language === 'zh' ? '删除' : 'Delete'}>
            <button
              onClick={e => {
                e.stopPropagation()
                onDelete()
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2 mt-1 text-[11px] text-text-muted/85">
                        <span className="flex items-center gap-1">
                          <Clock className="w-3 h-3" />
                          {getRelativeTime(branch.createdAt, language as 'zh' | 'en')}
                        </span>
                        <span>•</span>
                        <span>{branch.messages.length} {language === 'zh' ? '条消息' : 'msgs'}</span>
                      </div>
                    </>
                  )}
                </div>
