...
import {
    ...
    Layout, Plus, Trash2, GripVertical,
...
const LAYOUT_OPTIONS: { value: UILayout; label: { en: string; zh: string }; desc: { en: string; zh: string } }[] = [
    ...
    { value: 'canvas-centric', label: { en: 'Canvas Centric', zh: '画布为主' }, desc: { en: 'Freeform canvas for mind maps & notes', zh: '自由画布，支持思维导图与笔记' } },
    { value: 'research-centric', label: { en: 'Research Centric', zh: '研究为主' }, desc: { en: 'Literature, notes & AI analysis', zh: '文献阅读、笔记与AI分析三栏' } },
    { value: 'focus-centric', label: { en: 'Focus Mode', zh: '专注模式' }, desc: { en: 'Distraction-free writing & coding', zh: '无干扰写作与编码，番茄钟' } },
    { value: 'split-centric', label: { en: 'Split View', zh: '分屏对比' }, desc: { en: 'Side-by-side editing with chat', zh: '编辑器+终端+对话全功能分屏' } },
...
const CATEGORY_OPTIONS: { value: ScenarioCategory; label: { en: string; zh: string } }[] = [
    ...
    { value: 'research', label: { en: 'Research', zh: '研究' } },
    { value: 'communication', label: { en: 'Communication', zh: '沟通' } },
    { value: 'entertainment', label: { en: 'Entertainment', zh: '娱乐' } },
    { value: 'business', label: { en: 'Business', zh: '商业' } },
    { value: 'health', label: { en: 'Health', zh: '健康' } },
    { value: 'finance', label: { en: 'Finance', zh: '金融' } },
    { value: 'legal', label: { en: 'Legal', zh: '法律' } },
    { value: 'marketing', label: { en: 'Marketing', zh: '营销' } },
...
const SIDEBAR_COMPONENT_OPTIONS = [
    ...
    { id: 'notes', icon: 'StickyNote', label: 'Notes', labelZh: '笔记', component: 'NotesView' },
    { id: 'knowledge', icon: 'BookOpen', label: 'Knowledge', labelZh: '知识库', component: 'KnowledgeView' },
    { id: 'prompts', icon: 'Zap', label: 'Prompts', labelZh: '提示词库', component: 'PromptsView' },
    { id: 'tasks', icon: 'CheckSquare', label: 'Tasks', labelZh: '任务', component: 'TasksView' },
    { id: 'bookmarks', icon: 'Bookmark', label: 'Bookmarks', labelZh: '书签', component: 'BookmarksView' },
]
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                        {CATEGORY_OPTIONS.map(opt => (
                            <button
                                ...
                                className={`text-[11px] px-2 py-1 rounded-md transition-all ${category === opt.value ? 'bg-accent/15 text-accent border border-accent/30' : 'text-text-muted hover:text-text-primary hover:bg-surface-hover border border-transparent'}`}
                            >
                ...
                <div className="space-y-2">
                    <label className="text-[11px] font-medium text-text-muted uppercase tracking-wider flex items-center gap-1.5">
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        ...
                        <Button
                            variant="ghost"
                            size="sm"
                            className="h-5 text-[11px] gap-0.5 px-1.5"
                        ...
                        <div className="border border-border/30 rounded-lg p-1.5 bg-surface/30 space-y-0.5">
                            {availableSidebarItems.map(item => (
                                <button
                                    ...
                                    className="w-full text-left px-2 py-1.5 text-[12px] text-text-secondary hover:text-text-primary hover:bg-surface-hover rounded transition-colors"
                                >
                    ...
                    <div className="space-y-1">
                        {sidebarItems.map((item) => (
                            <div key={item.id} className="flex items-center gap-1.5 px-2 py-1.5 rounded-md bg-surface/30 border border-border/20">
                                <GripVertical className="w-3 h-3 text-text-muted/75 flex-shrink-0" />
                                <span className="text-[12px] text-text-primary flex-1 truncate">
                                    {language === 'zh' ? item.labelZh : item.label}
                                </span>
                                <span className="text-[10px] text-text-muted/85">{item.id}</span>
                                <button
                                    onClick={() => handleRemoveSidebarItem(item.id)}
                                    className="p-0.5 text-text-muted/85 hover:text-red-400 transition-colors"
                                >
                        ...
                        {sidebarItems.length === 0 && (
                            <p className="text-[11px] text-text-muted/85 text-center py-2">
