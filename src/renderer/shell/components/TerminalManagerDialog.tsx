...
import { ActionButton, TextField, OverlayDialog } from '@/renderer/components/ui'
import { shellRegistryService } from '../services/terminalRegistry'
import { DEFAULT_REMOTE_PORT } from '../types/terminalTypes'
import type { AvailableShell, RemoteServerConfig, ShellLink, ShellPreset } from '../types/terminalTypes'
...
export function TerminalManagerDialog({
  ...
  return (
    <OverlayDialog isOpen={isOpen} onClose={onClose} title="Shell 管理" size="5xl">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[240px_minmax(0,280px)_minmax(0,1fr)] gap-4 min-h-[520px]">
          <div className="rounded-2xl border border-border/60 bg-surface/30 p-3 space-y-3">
            ...
            <div className="rounded-2xl border border-border/60 bg-background/30 p-3 space-y-2">
              ...
              <ActionButton variant="ghost" size="sm" className="w-full justify-start" onClick={() => createItem('preset')}><Plus className="w-3.5 h-3.5 mr-1" />Preset</ActionButton>
              <ActionButton variant="ghost" size="sm" className="w-full justify-start" onClick={() => createItem('directory')}><FolderOpen className="w-3.5 h-3.5 mr-1" />目录链接</ActionButton>
              <ActionButton variant="ghost" size="sm" className="w-full justify-start" onClick={() => createItem('remote')}><Server className="w-3.5 h-3.5 mr-1" />服务器</ActionButton>
              <ActionButton variant="ghost" size="sm" className="w-full justify-start" onClick={() => createItem('command')}><Sparkles className="w-3.5 h-3.5 mr-1" />命令</ActionButton>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-border/60 bg-surface/30 p-3">
            <div className="flex items-center justify-between gap-2 px-1 pb-3 border-b border-border/40">
              ...
              {activeSection !== 'overview' && <ActionButton variant="ghost" size="sm" onClick={() => createItem(activeSection)}><Plus className="w-3.5 h-3.5 mr-1" />新增</ActionButton>}
            </div>
          ...
          <div className="rounded-2xl border border-border/60 bg-surface/30 p-4">
            {selectedItem.kind === 'overview' && <div className="space-y-5"><div><div className="text-sm font-medium text-text-primary">默认 Shell</div><div className="text-xs text-text-muted mt-1">用于新建终端以及未指定 Shell 的入口</div></div><select value={formDefaultShell} onChange={(e) => setFormDefaultShell(e.target.value)} className="w-full rounded-xl border border-border/60 bg-background/60 px-3 py-2 text-sm text-text-primary outline-none"><option value="">系统默认 Shell</option>{availableShells.map((shell) => <option key={`${shell.label}-${shell.path}`} value={shell.path}>{shell.label} {shell.path ? `(${shell.path})` : ''}</option>)}</select><div><div className="text-sm font-medium text-text-primary mb-2">命令建议</div><div className="flex flex-wrap gap-2">{suggestedCommands.map((command) => <ActionButton key={command} variant="ghost" size="sm" onClick={() => createItem('command', command)}>{command}</ActionButton>)}</div></div></div>}
            {selectedPreset && <div className="space-y-4"><div className="flex items-center justify-between gap-2"><div><div className="text-sm font-medium text-text-primary">Preset 编辑</div><div className="text-xs text-text-muted">配置启动目录、参数和显示方式</div></div><div className="flex items-center gap-1"><ActionButton variant="ghost" size="icon" onClick={() => moveSelected(-1)}><ArrowUp className="w-4 h-4" /></ActionButton><ActionButton variant="ghost" size="icon" onClick={() => moveSelected(1)}><ArrowDown className="w-4 h-4" /></ActionButton><ActionButton variant="ghost" size="sm" onClick={() => updatePreset(selectedPreset.id, { favorite: !selectedPreset.favorite })}><Star className={`w-4 h-4 ${selectedPreset.favorite ? 'fill-current text-yellow-400' : ''}`} /></ActionButton><ActionButton variant="ghost" size="icon" onClick={removeSelected}><Trash2 className="w-4 h-4" /></ActionButton></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><TextField value={selectedPreset.name} onChange={(e) => updatePreset(selectedPreset.id, { name: e.target.value })} placeholder="名称" /><TextField value={selectedPreset.group || ''} onChange={(e) => updatePreset(selectedPreset.id, { group: e.target.value })} placeholder="分组（可选）" /><TextField value={selectedPreset.cwd || ''} onChange={(e) => updatePreset(selectedPreset.id, { cwd: e.target.value })} placeholder="工作目录（可选）" /><TextField value={selectedPreset.args?.join(' ') || ''} onChange={(e) => updatePreset(selectedPreset.id, { args: e.target.value.trim() ? e.target.value.trim().split(/\s+/) : undefined })} placeholder="启动命令（可选）" /></div><select value={selectedPreset.shellPath || ''} onChange={(e) => updatePreset(selectedPreset.id, { shellPath: e.target.value })} className="w-full rounded-xl border border-border/60 bg-background/60 px-3 py-2 text-sm text-text-primary outline-none"><option value="">默认 Shell</option>{availableShells.map((shell) => <option key={`${selectedPreset.id}-${shell.path}`} value={shell.path}>{shell.label}</option>)}</select><label className="flex items-center gap-2 text-sm text-text-secondary px-1"><input type="checkbox" checked={selectedPreset.visibleInMenu !== false} onChange={(e) => updatePreset(selectedPreset.id, { visibleInMenu: e.target.checked })} />在菜单中显示</label></div>}
            {selectedLink && <div className="space-y-4"><div className="flex items-center justify-between gap-2"><div><div className="text-sm font-medium text-text-primary">{selectedLink.type === 'remote' ? '服务器编辑' : selectedLink.type === 'command' ? '命令编辑' : '链接编辑'}</div><div className="text-xs text-text-muted">当前入口的详细配置</div></div><div className="flex items-center gap-1"><ActionButton variant="ghost" size="icon" onClick={() => moveSelected(-1)}><ArrowUp className="w-4 h-4" /></ActionButton><ActionButton variant="ghost" size="icon" onClick={() => moveSelected(1)}><ArrowDown className="w-4 h-4" /></ActionButton><ActionButton variant="ghost" size="sm" onClick={() => updateLink(selectedLink.id, { favorite: !selectedLink.favorite })}><Star className={`w-4 h-4 ${selectedLink.favorite ? 'fill-current text-yellow-400' : ''}`} /></ActionButton><ActionButton variant="ghost" size="icon" onClick={removeSelected}><Trash2 className="w-4 h-4" /></ActionButton></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><TextField value={selectedLink.name} onChange={(e) => updateLink(selectedLink.id, { name: e.target.value })} placeholder="名称" /><TextField value={selectedLink.group || ''} onChange={(e) => updateLink(selectedLink.id, { group: e.target.value })} placeholder="分组（可选）" /><select value={selectedLink.type} onChange={(e) => updateLink(selectedLink.id, { type: e.target.value as ShellLink['type'], target: '', cwd: '', remote: e.target.value === 'remote' ? normalizeRemote(selectedLink.remote) : undefined })} className="w-full rounded-xl border border-border/60 bg-background/60 px-3 py-2 text-sm text-text-primary outline-none"><option value="directory">目录</option><option value="local-shell">本地 Shell</option><option value="command">常用命令</option><option value="remote">远程服务器</option></select><select value={selectedLink.shellPath || ''} onChange={(e) => updateLink(selectedLink.id, { shellPath: e.target.value })} className="w-full rounded-xl border border-border/60 bg-background/60 px-3 py-2 text-sm text-text-primary outline-none"><option value="">默认 Shell</option>{availableShells.map((shell) => <option key={`${selectedLink.id}-${shell.path}`} value={shell.path}>{shell.label}</option>)}</select></div>{selectedLink.type === 'remote' ? <div className="grid grid-cols-1 md:grid-cols-2 gap-3"><TextField value={normalizeRemote(selectedLink.remote).host} onChange={(e) => updateRemote(selectedLink.id, { host: e.target.value })} placeholder="Host" /><TextField value={normalizeRemote(selectedLink.remote).username || ''} onChange={(e) => updateRemote(selectedLink.id, { username: e.target.value })} placeholder="Username" /><TextField type="number" value={String(normalizeRemote(selectedLink.remote).port || DEFAULT_REMOTE_PORT)} onChange={(e) => updateRemote(selectedLink.id, { port: Number(e.target.value) || DEFAULT_REMOTE_PORT })} placeholder="Port" /><TextField type="password" value={normalizeRemote(selectedLink.remote).password || ''} onChange={(e) => updateRemote(selectedLink.id, { password: e.target.value })} placeholder="密码（可选）" /><TextField value={normalizeRemote(selectedLink.remote).remotePath || ''} onChange={(e) => updateRemote(selectedLink.id, { remotePath: e.target.value })} placeholder="远程目录（可选）" /><div className="md:col-span-2"><TextField value={normalizeRemote(selectedLink.remote).privateKeyPath || ''} onChange={(e) => updateRemote(selectedLink.id, { privateKeyPath: e.target.value })} placeholder="私钥路径（可选）" /></div></div> : selectedLink.type === 'command' ? <div className="grid grid-cols-1 md:grid-cols-2 gap-3"><TextField value={selectedLink.target} onChange={(e) => updateLink(selectedLink.id, { target: e.target.value })} placeholder="命令内容，例如 npm run dev" /><TextField value={selectedLink.cwd || ''} onChange={(e) => updateLink(selectedLink.id, { cwd: e.target.value })} placeholder="工作目录（可选）" /></div> : <TextField value={selectedLink.target} onChange={(e) => updateLink(selectedLink.id, { target: e.target.value })} placeholder={selectedLink.type === 'local-shell' ? 'Shell 路径' : '目录路径'} />}<label className="flex items-center gap-2 text-sm text-text-secondary px-1"><input type="checkbox" checked={selectedLink.visibleInMenu !== false} onChange={(e) => updateLink(selectedLink.id, { visibleInMenu: e.target.checked })} />在菜单中显示</label></div>}
          </div>
        </div>

        <div className="flex items-center justify-end gap-2 pb-1"><ActionButton variant="ghost" onClick={onClose}>取消</ActionButton><ActionButton onClick={handleSave} disabled={saving}>{saving ? '保存中...' : '保存'}</ActionButton></div>
      </div>
    </OverlayDialog>
  )
}
